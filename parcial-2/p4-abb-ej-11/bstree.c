...
void *bstree_k_esimo_menor(BSTree arbol, int k) {
  ...
  Cada vez que se visita un dato, se reduce el contador en 1 y cuando este se
  haga 0, el recorrido habra llegado al nodo buscado. Se usa un puntero a un
  entero porque todas las llamadas recursivas deben tener acceso a la misma
  variable contador (recordar que en c los parámetros se pasan por valor) */
  return bstree_k_esimo_menor_aux(arbol, &k);
}

void *bstree_k_esimo_menor_v2(BSTree arbol, int *k) {
  if (bstree_es_vacio(arbol))
    return NULL;
  void *dato = bstree_k_esimo_menor_v2(arbol->izq, k);
  if (dato != NULL)
    return dato;
  if (*k == 0)
    return arbol->dato;
  --(*k);
  return bstree_k_esimo_menor_v2(arbol->der, k);
}