...
/**
 * Elimina un dato del arbol, manteniendo la
 * propiedad del arbol de busqueda binaria
 */

BSTree bstree_eliminar(BSTree arbol, void *dato, FuncionComparadora cmp,
  ...
  if (!cmp(arbol->dato, dato)) {
    ...
    if (!arbol->der || !arbol->izq) {
      replacement = arbol->der ? arbol->der : arbol->izq;
      arbol->izq = arbol->der = NULL;
      bstree_destruir(arbol, destroy);
      arbol = replacement;
    } else {
      replacement = arbol->der;
      for (; replacement->izq; replacement = replacement->izq)
        ;
      dato = arbol->dato;
      arbol->dato = replacement->dato;
      replacement->dato = dato;
...
BSTree bstree_eliminar_v2(BSTree arbol, void *dato, FuncionComparadora cmp, FuncionDestructora destroy) {
    if (arbol == NULL)
        return NULL;
    ...
    if (!cmp(arbol->dato, dato)) {
        ...
        if (!arbol->der || !arbol->izq) {
            ...
            BSTree* replacementPtr = &(arbol->der);
            replacement = arbol->der;

            if (replacement->izq) {
                for (; replacement->izq->izq; replacement = replacement->izq);
                replacementPtr = &(replacement->izq);
                replacement = replacement->izq;
            }

            dato = arbol->dato;
            arbol->dato = replacement->dato;
            replacement->dato = dato;
            *replacementPtr = bstree_eliminar_v2(replacement, dato, cmp, destroy);
        }
    }
    else if (cmp(arbol->dato, dato) < 0)
        arbol->der = bstree_eliminar_v2(arbol->der, dato, cmp, destroy);
    else 
        arbol->izq = bstree_eliminar_v2(arbol->izq, dato, cmp, destroy);
