...
static int avl_nodo_factor_balance(AVL_Nodo *raiz) {
  assert(raiz != NULL);
  int factor =
      avl_nodo_altura(raiz->der) -
      avl_nodo_altura(raiz->izq); 
...
static AVL_Nodo *avl_nodo_insertar(AVL_Nodo *raiz, void *dato,
  ...
  if (raiz == NULL) // insertamos el nuevo elemento
    ...
    // el dato debe ir en el subarbol izq
    ...
    // el dato debe ir en el subarbol der
...
AVL_Nodo *avl_min_valor_nodo(AVL_Nodo *nodo) {
  AVL_Nodo *current = nodo;
...
AVL_Nodo *avl_eliminar(AVL_Nodo *raiz, void *dato, FuncionCopiadora copia,
                       FuncionComparadora comp) {
  ...
  if (comp(dato, raiz->dato) < 0) {
    raiz->izq = avl_eliminar(raiz->izq, dato, copia, comp);
  }

  else if (comp(dato, raiz->dato) > 0)
    raiz->der = avl_eliminar(raiz->der, dato, copia, comp);
    ...
    if ((raiz->izq == NULL) || (raiz->der == NULL)) {
      ...
      AVL_Nodo *temp = avl_min_valor_nodo(raiz->der);

      raiz->dato = copia(temp->dato);

      raiz->der = avl_eliminar(raiz->der, temp->dato, copia, comp);
    }
  }
