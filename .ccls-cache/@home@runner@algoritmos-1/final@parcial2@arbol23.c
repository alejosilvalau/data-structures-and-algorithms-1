...
static int insertar_impl(Arbol23 t, int x, int *out_x, Arbol23 *out_l,
                         Arbol23 *out_r) {
...
// Recorre el arbol23 in-order way
static void arbol23_recorrer(Arbol23 raiz) {
  if (raiz != NULL) {
    arbol23_recorrer(raiz->left);
    printf("Nodo: k1 = %d k2 = %d \n", raiz->k1, raiz->k2);
    arbol23_recorrer(raiz->middle);
		if (raiz->right != CENTINELA) arbol23_recorrer(raiz->right);
  }
}

int main() {
  Arbol23 arbol = arbol23_crear();
	
	arbol = arbol23_insertar(arbol, 1);
  arbol = arbol23_insertar(arbol, 2);
	arbol = arbol23_insertar(arbol, 3);
	arbol = arbol23_insertar(arbol, 4);
	arbol = arbol23_insertar(arbol, 5);
	arbol = arbol23_insertar(arbol, 6);
	arbol = arbol23_insertar(arbol, 7);
	
	arbol23_recorrer(arbol);
