...
/**
 * Ejercicio 5: 
 * Búsqueda por extensión recursiva y su función auxiliar que visita solo los
 * nodos que se encuentran a una profundidad determinada.
 */
...
int aux(BTree arbol, FuncionVisitante visit, int profundidad){
  if(arbol == NULL){
    ...
    return aux(arbol->left, visit, profundidad-1) + aux(arbol->right, visit,
    profundidad-1);
  }
}
